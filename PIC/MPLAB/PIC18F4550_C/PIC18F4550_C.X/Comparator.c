...
unsigned char cmcon_flush;     //to read CMCON
void main()
{
...
//comparator interrupt config function
void interrupt_config(void)
{
...
void interrupt isr(void)
{
    asm("CALL _comparator_isr");
}
//comparator ISR function
void comparator_isr(void)
{
    RD1=~RD1;
    cmcon_flush=CMCON;    //CMCON must be read or interrupt cannot be cleared
    CMIF=0;      //clear interrupt
}